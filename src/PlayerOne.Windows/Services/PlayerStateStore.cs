...
public static class PlayerStateStore {
 ...
 public static void AddRecent(DeviceAuth a,long? p,RecentEntry e){var x=Recents(a,p);x.RemoveAll(i=>SameRecent(i,e));x.Insert(0,e with{UpdatedAt=DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()});Write(PathFor(a,p,"recents"),x.Take(120));}
 public static RecentEntry? ResumeEntry(DeviceAuth a,long? p,string? streamUrl)=>string.IsNullOrWhiteSpace(streamUrl)?null:Recents(a,p).FirstOrDefault(x=>x.StreamUrl==streamUrl&&Meaningful(x));
 public static void RemoveRecent(DeviceAuth a,long? p,string? streamUrl){if(string.IsNullOrWhiteSpace(streamUrl))return;var x=Recents(a,p);x.RemoveAll(i=>i.StreamUrl==streamUrl);Write(PathFor(a,p,"recents"),x);}
 public static void ClearRecents(DeviceAuth a,long? p)=>Write(PathFor(a,p,"recents"),Array.Empty<RecentEntry>());
 public static void ClearFavorites(DeviceAuth a,long? p)=>Write(PathFor(a,p,"favorites"),Array.Empty<FavoriteEntry>());
 public static bool IsFavorite(DeviceAuth a,long? p,string type,string id)=>Favorites(a,p).Any(x=>x.Type==type&&x.Id==id);
 ...
 static bool SameRecent(RecentEntry a,RecentEntry b){if(!string.IsNullOrWhiteSpace(a.StreamUrl)&&!string.IsNullOrWhiteSpace(b.StreamUrl))return a.StreamUrl==b.StreamUrl;return a.Type==b.Type&&a.Title==b.Title&&a.Subtitle==b.Subtitle;}
