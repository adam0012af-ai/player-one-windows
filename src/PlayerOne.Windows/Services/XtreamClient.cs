...
public sealed class XtreamClient {
 ...
 public async Task<List<XtreamItem>> LoadAsync(string host,string user,string pass,string kind){
  ...
  var cats=JsonDocument.Parse(await http.GetStringAsync($"{baseUrl}&action={catAction}")).RootElement;var map=new Dictionary<string,string>();foreach(var c in cats.EnumerateArray()){var id=Get(c,"category_id");if(id.Length>0)map[id]=Get(c,"category_name");}
